...
arrayshift!(result, array, shift, :: NonPeriodic) = padshift!(result, array, shift)
arrayshift!(result, array, shift, :: Periodic)   = circshift!(result, array, shift)

autocorr3_norm(array :: AbstractArray, :: Any, :: Any, :: Periodic) = length(array)
function autocorr3_norm(array :: AbstractArray, s1, s2, :: NonPeriodic)
...
function crosscorr3_plane(array1 :: AbstractArray{<: Any, N},
    ...
    function cc(shift2, shift3)
        arrayshift!(rot2, array2, shift2, mode)
        arrayshift!(rot3, array3, shift3, mode)
        sum(op(rot1, rot2, rot3)) /
            autocorr3_norm(array1, shift2, shift3, mode)
    end
...
autocorr3_plane(array :: AbstractArray, op, mode, ps1, ps2) =
    crosscorr3_plane(array, array, array, op, mode, ps1, ps2)

"""
    s3(array, ps1, ps2[, mode = NonPeriodic()])
...
with the choose of `mode` argument.
...
"""
function s3(array :: AbstractArray, ps1, ps2; mode :: AbstractMode = NonPeriodic())
    op(x, y, z) = @. x * y * z
    return autocorr3_plane(array, op, mode, ps1, ps2)
end
